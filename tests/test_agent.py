...
class TestSshAgent(unittest.TestCase):
    ...
    private_key = os.path.join(ansible_ws_tests.ANSIBLE_WS_PATH_TEST, 'data', 'agent', 'wapi_rsa')
    public_key = os.path.join(ansible_ws_tests.ANSIBLE_WS_PATH_TEST, 'data', 'agent', 'wapi_rsa.pub')

    def test(self):

      ssh_agent = SshAgent()
    #   pprint.pprint(ssh_agent.env)
      ssh_agent.load_key(self.private_key, 'wapi')
